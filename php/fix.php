...
if (mysqli_num_rows($qInfo) > 0) {
	while ($row = mysqli_fetch_assoc($qInfo)){
		...
		echo '<form action="updateQnum.php?textId=' . $_GET["textId"] . '" method="post">Question: <input type="text" name="qNum" value="' .$row["questionNumGuess"] . '" size="3">&nbsp&nbsp&nbsp&nbsp<input type="submit" value="Fix Q Num"></form>';
		echo '<hr/>';
		echo '<form action="shiftQuestions.php?textId=' . $_GET["textId"] . '" method="post"><input type="submit" value="Shift Questions Up"></form><hr/>'; 
